...
public:
                                         ...
                                         cv::Mat projection, float radius2D, float radius3D)
  {
    ...
    std::cout << "z camara: " << radius3D * focal_len / radius2D <<std::endl;
    ...
    {
      // t_x:
      translation[INDEX::X] += (image[i].x - principal_x) * (velodyne[i].z + translation[INDEX::Z]) / focal_len - velodyne[i].x;
      
      // t_y:
      translation[INDEX::Y] += (image[i].y - principal_y) * (velodyne[i].z + translation[INDEX::Z]) / focal_len - velodyne[i].y;
